...
#define STEPS 1000 // defalut is 1000
...
float calculate_accuracy(mnist_dataset_t * dataset, neural_network_t * network)
{
   ...
   FILE * activations_file = fopen("activation","wb");
...
void save(neural_network_t* network)
{
    ...
    printf("Etape 01\n");
    FILE * bias_file = fopen("../ImageProcessing/bias","wb");
    if(bias_file == NULL)
    {
    ...
    fclose(bias_file);
    FILE * weight_file = fopen("../ImageProcessing/weights","wb");
    if(weight_file == NULL)
    {
...
void load(neural_network_t* network, float ** activations)
{
    ...
    FILE * bias_file = fopen("../ImageProcessing/bias","rb");
    //rewind(bias_file);
    ...
    fclose(bias_file);
    printf("Etape 2 Load\n");
    FILE * weight_file = fopen("../ImageProcessing/weights","rb");
    //rewind(weight_file);
    int i = 0;
    int j = 0;
    int cpt = 0;
    ...
    FILE * activation_file = fopen("../ImageProcessing/activation","rb");
...
{
    printf("Step is set to : %i should be 1000",STEPS);
